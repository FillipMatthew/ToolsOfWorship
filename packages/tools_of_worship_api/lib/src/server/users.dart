...
import 'helpers/account_management.dart';
import 'helpers/email_relay.dart';
import 'helpers/google_api.dart';
import 'interfaces/users_data_provider.dart';
...
class ApiUsers {
  ...
  Future<Response> _authenticate(Request request) async {
    ...
    if (authHeader != null) {
      ...
      try {
        ...
        user = await _signIn(SignInType.fromJson(signinData['signInType']),
            signinData['accountId'], signinData['password']);
    ...
    final String token =
        AccountManagement.signToken(user.id, Duration(minutes: 15));
  ...
  Future<Response> _signup(Request request) async {
    ...
    try {
      ...
      String authDetails = AccountManagement.hashPassword(password);
  ...
  Future<Response> _verifyEmail(Request request) async {
    ...
    String token = AccountManagement.decryptToken(encryptedToken);
    String? data = AccountManagement.verifyToken(token);
    if (data == null) {
      return Response.forbidden('Invalid token.');
    }
  ...
  Future<User?> _signIn(
    ...
    if (signInType == SignInType.localUser) {
      ...
      if (!AccountManagement.validatePassword(password, saltHash)) {
        return null;
      }
      ...
      Map<String, String> signInData =
          await GoogleApi.authenticateToken(accountId);
  ...
  Future<bool> _sendVerificationMail(
    ...
    String token = AccountManagement.signToken(jsonData, Duration(minutes: 15));
    String encryptedToken = AccountManagement.encryptToken(token);
